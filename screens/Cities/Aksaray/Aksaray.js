...
_goToYosemite() {
  ...
  createMapLink({ provider: 'google', end: dbb.item.cities['Aksaray'][city].name })
}
...
handleGetDirections = () => {
  ...
  componentDidMount() {
    ...
    fetch("https://api.openweathermap.org/data/2.5/weather?q=aksaray,tr&lang="+this.state.lang+"&appid=e856bec0ffe36d86b4d8af3ca4626e0a")
    .then(res=>{
      return res.json();
    }).then(responseData=>{
...
<Modal visible={visible}  transparent={true}>
                ...
                <ImageViewer renderHeader={()=>abc()} enableSwipeDown={true}onSwipeDown={()=>this.setState({visible:false})} enableImageZoom={true} index={imageindex} imageUrls={dbb.item.cities['Aksaray'][city].DATA}>
...
<ImageBackground source={{ uri:dbb.item.cities['Aksaray'][city].backgroundPhoto}} resizeMode="cover" style={styles.image}>
  ...
  {dbb.item.cities['Aksaray'][city].name}
     ...
     :
     <Text style={{padding:5,color:color,fontWeight:'bold',fontFamily:fontfam,fontSize:30,alignItems:'center',justifyContent:'center'}}>
  {dbb.item.cities['Aksaray'][city].name3}
     </Text>
       }
     ...
     {lang=='tr' 
     ?
     dbb.item.cities['Aksaray'][city].name2=='-'
     ? <View></View>
 ...
 :  <View style={{alignItems:'center'}}> 
  ...
  {dbb.item.cities['Aksaray'][city].name2}
     ...
     :
     dbb.item.cities['Aksaray'][city].name4=='-'
     ? <View></View>
...
<TouchableOpacity 
onPress={()=>Linking.openURL("https://www.accuweather.com/tr/tr/aksaray/316744/weather-forecast/316744")} 
style={{alignSelf:'center',fontWeight:'600',borderBottomColor:color,borderBottomWidth:3,borderLeftColor:color,borderLeftWidth:2,backgroundColor:`#00000070`,borderRadius:5,alignItems:'center',justifyContent:'center',marginTop:55}}> 
...
<TouchableOpacity 
onPress={()=>Linking.openURL("https://www.accuweather.com/tr/tr/aksaray/316744/weather-forecast/316744")} 
style={{alignSelf:'center',fontWeight:'600',borderBottomColor:color,borderBottomWidth:3,borderLeftColor:color,borderLeftWidth:2,backgroundColor:`#00000070`,borderRadius:5,alignItems:'center',justifyContent:'center'}}> 
...
<FlatList
        data={dbb.item.cities['Aksaray'][city].DATA}
...
<ActionButton.Item  size={75}buttonColor={color}  onPress={()=>Linking.openURL(dbb.item.cities['Aksaray'][city].morePhoto)}>
...
<ActionButton.Item  size={75}buttonColor={color}  onPress={()=>{lang=='tr' ? this.props.navigation.navigate(`AksarayInfo`,{city:city,color:color,color2:color2,fontfam:fontfam}):Linking.openURL("https://en.wikipedia.org/wiki/"+dbb.item.cities['Aksaray'][city].name)}}>
