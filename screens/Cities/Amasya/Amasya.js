...
_goToYosemite() {
  ...
  createMapLink({ provider: 'google', end: dbb.item.cities['Amasya'][city].name })
}
...
handleGetDirections = () => {
  ...
  componentDidMount() {
    ...
    fetch("https://api.openweathermap.org/data/2.5/weather?q=Amasya,tr&lang="+this.state.lang+"&appid=e856bec0ffe36d86b4d8af3ca4626e0a")
    .then(res=>{
      return res.json();
    }).then(responseData=>{
...
<Modal visible={visible}  transparent={true}>
                ...
                <ImageViewer renderHeader={()=>abc()} enableSwipeDown={true}onSwipeDown={()=>this.setState({visible:false})} enableImageZoom={true} index={imageindex} imageUrls={dbb.item.cities['Amasya'][city].DATA}>
...
<ImageBackground source={{ uri:dbb.item.cities['Amasya'][city].backgroundPhoto}} resizeMode="cover" style={styles.image}>
  ...
  {dbb.item.cities['Amasya'][city].name}
     </Text>
     :
     <Text style={{padding:5,color:color,fontWeight:'bold',fontFamily:fontfam,fontSize:30,alignItems:'center',justifyContent:'center'}}>
  {dbb.item.cities['Amasya'][city].name3}
     </Text>
       }
     ...
     {lang=='tr' 
     ?
     dbb.item.cities['Amasya'][city].name2=='-'
     ? <View></View>
 ...
 :  <View style={{alignItems:'center'}}> 
  ...
  {dbb.item.cities['Amasya'][city].name2}
     ...
     :
     dbb.item.cities['Amasya'][city].name4=='-'
     ? <View></View>
...
<TouchableOpacity 
onPress={()=>Linking.openURL("https://www.accuweather.com/tr/tr/amasya/316858/weather-forecast/316858")} 
...
<FlatList
        data={dbb.item.cities['Amasya'][city].DATA}
...
<ActionButton.Item  size={75}buttonColor={color}  onPress={()=>Linking.openURL(dbb.item.cities['Amasya'][city].morePhoto)}>
...
<ActionButton.Item  size={75}buttonColor={color}  onPress={()=>{lang=='tr' ? this.props.navigation.navigate(`AmasyaInfo`,{city:city,color:color,color2:color2,fontfam:fontfam}):Linking.openURL("https://en.wikipedia.org/wiki/"+dbb.item.cities['Amasya'][city].name)}}>
